...
### 48. Исправление расчёта платных дней хранения + автопересчёт (cron)

**Дата:** 10 февраля 2026 г. (ночь)

#### Проблема 1: Расхождение платных дней между списком и карточкой ТС

**Корневая причина:** В списке автомобилей `days_display()` рассчитывал дни динамически по текущей дате, а в сводке карточки `services_summary_display()` брал сохранённое значение `obj.days`, которое обновлялось только при сохранении модели.

**Решение:**
1. ✅ `services_summary_display()` теперь считает платные дни динамически (как и `days_display`)

**Файл:** `core/admin/car.py`

#### Проблема 2: Стоимость услуги «Хранение» в CarService не обновлялась

**Корневая причина:** `CarService.custom_price` для «Хранения» обновлялась только при `Car.save()` (через `update_days_and_storage` → `_update_storage_service_price`). Если машина стоит на складе и никто её не пересохраняет — цена устаревает (например, 5 EUR за 1 день вместо 15 EUR за 3 дня).

**Решение — тройная защита:**
1. ✅ `CarAdmin.change_view()` — при открытии карточки ТС автоматически пересчитывает дни и цену хранения, если данные устарели
2. ✅ Команда `recalculate_storage` улучшена: обрабатывает только активные (не переданные) машины, обновляет `CarService.custom_price`, поддерживает `--verbose`, `--all`
3. ✅ Cron-задача `0 6 * * *` — ежедневный пересчёт в 06:00

**Файлы:**
- `core/admin/car.py` — динамический расчёт в сводке + auto-update в change_view
- `core/management/commands/recalculate_storage.py` — улучшенная команда

---

### 49. Исправление CHECK constraint для CarService (COMPANY)

**Дата:** 12 февраля 2026 г.

**Проблема:** При добавлении услуги компании к автомобилю возникала ошибка `core_carservice_service_type_check`. Старая миграция (0062, времён Django 2.x) создала CHECK constraint, разрешающий только `LINE`, `CARRIER`, `WAREHOUSE`. Тип `COMPANY` был добавлен позже (миграция 0101), но Django 5.x не обновляет такие constraints автоматически.

**Решение:**
- ✅ Миграция `0116_fix_carservice_service_type_check.py` — удаляет старый и создаёт новый constraint с 4 значениями

**Файлы:**
- `core/migrations/0116_fix_carservice_service_type_check.py`

---

### 50. Исправление прав доступа к media и cron на VPS

**Дата:** 12 февраля 2026 г.

**Проблема:** Загрузка фото с Google Drive не работала на VPS — `Permission denied` на папку `media/container_photos/`. Cron-задачи также не работали — `ModuleNotFoundError: No module named 'django'`.

**Корневые причины:**
1. Папки `media/` были созданы от root, а gunicorn работает от `www-root`
2. Cron использовал `source .venv/bin/activate; python` — не работает в неинтерактивном shell
3. Все cron-задачи были в crontab root'а, создавая файлы с владельцем root

**Решение:**
1. ✅ `chown -R www-root:www-root media/` и `/var/log/logist2/`
2. ✅ Все cron-задачи перенесены в crontab пользователя `www-root`
3. ✅ Все cron-задачи используют `.venv/bin/python` напрямую
4. ✅ Обновлён `sync_photos_cron.sh` — правильные пути, актуальная документация

---

### 51. Автовозы — автопередача авто, кнопка "Загружен", цветные инвойсы

**Дата:** 12 февраля 2026 г.

#### Автосмена статуса авто при изменении статуса автовоза

При смене статуса автовоза на "Загружен", "В пути" или "Доставлен" — все автомобили автовоза автоматически получают статус "Передан" с датой передачи.

**Реализация:**
- ✅ Сигнал `autotransport_post_save` в `core/signals.py` расширен
- ✅ Функция `_mark_cars_as_transferred()` — обновляет статус и дату передачи
- ✅ Поддержка кастомной даты через атрибут `_transfer_date_override`

#### Кнопка "Загружен" + поле даты на странице списка автовозов

- ✅ AJAX-кнопка "🚛 Загружен" с полем даты для каждого автовоза со статусом FORMED/DRAFT
- ✅ При пустой дате — используется текущая; при заполненной — указанная
- ✅ Endpoint `mark-loaded/` в `AutoTransportAdmin.get_urls()`
- ✅ Шаблон `templates/admin/core/autotransport/change_list.html` с JS-обработчиком

#### Цветовая индикация кнопки "Инвойсы"

- ✅ Серая — инвойсы не выставлены
- ✅ Красная — выставлены, не оплачены (ISSUED/OVERDUE)
- ✅ Жёлтая — частично оплачены (PARTIALLY_PAID)
- ✅ Зелёная — оплачены (PAID)
- ✅ Градиент — несколько инвойсов с разными статусами (кнопка делится на цветные сегменты)

#### Скрытие доставленных автовозов и массовое действие

- ✅ Доставленные автовозы скрыты по умолчанию (через `get_changelist_instance` + `get_queryset`)
- ✅ Видны при выборе "Доставлен" в фильтре по статусу
- ✅ Mass action "🚛 Присвоить статус Доставлен" — с автопередачей авто

#### Кликабельные автомобили в автовозе и инвойсах

- ✅ Select2 теги авто — клик по тексту открывает карточку в новой вкладке
- ✅ VIN в таблице позиций инвойса — кликабельная ссылка
- ✅ Цвета по статусу сохранены (FLOATING/IN_PORT/UNLOADED/TRANSFERRED)

**Файлы:**
- `core/admin/partners.py` — AutoTransportAdmin (get_urls, mark_loaded_view, save_model, actions_display, get_queryset, mark_delivered)
- `core/signals.py` — autotransport_post_save, _mark_cars_as_transferred
