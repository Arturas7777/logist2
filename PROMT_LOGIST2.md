...
Для входящих инвойсов (аренда, коммуналка, прочие расходы) сумма задаётся вручную:

1. **Поле "Сумма к оплате"** — вверху формы, рядом с "Номер счёта контрагента"
   - При выбранных авто: readonly, сумма считается через AJAX (`/admin/core/newinvoice/calc-cars-total/`)
   - Без авто: ручной ввод — `total` автоматически становится долгом компании

2. **Блок "Позиции вручную"** — динамические строки (описание + кол-во + цена)
   - Показывается только когда нет авто
   - При добавлении позиций — сумма вверху автоматически пересчитывается
   - Данные передаются через `manual_items_json` (JSON → `InvoiceItem`)

3. **Обработка на бэкенде** (`NewInvoiceAdmin._handle_manual_items()`):
   - Ручные позиции → `InvoiceItem` для каждой строки
   - Только `manual_total` без позиций → одна общая `InvoiceItem`
   - `calculate_totals()` → `save()` → `remaining_amount` = долг
...
### Дашборд компании ⭐ НОВОЕ (08.02.2026)

Полноценный дашборд с оперативными и финансовыми KPI, графиками и таблицами.

**URL:** `/admin/dashboard/` (иконка 📊 в навигации админки)
...
3. **Графики (Chart.js 4.x):** доходы/расходы за 6 мес. (bar), авто по статусам (doughnut), инвойсы по статусам (doughnut)
4. **Таблицы:** последние 15 транзакций + последние 15 инвойсов
5. **Быстрые действия:** создание инвойса/транзакции, навигация
...
- `core/views.py` — `company_dashboard()` делегирует в `DashboardService`
- `templates/admin/company_dashboard.html` — шаблон с Chart.js
- `templates/admin/base_site.html` — иконка 📊 в навигации
- `logist2/urls.py` — URL `/admin/dashboard/`
...
Расширяемая система интеграции с банковскими API. На данный момент подключён Revolut Business.

```
core/models_banking.py          # BankConnection, BankAccount, BankTransaction
core/services/revolut_service.py # RevolutService — API-клиент
core/admin_banking.py           # Django Admin для банковских моделей
core/management/commands/
├── setup_revolut.py            # Помощник первоначальной настройки
└── sync_bank_accounts.py       # Команда для cron-синхронизации
```
...
value = conn.client_id             # расшифровывает из _client_id
```

### Revolut API

- **Base URL:** `https://b2b.revolut.com` (production), `https://sandbox-b2b.revolut.com` (sandbox)
- **Auth:** JWT + refresh_token → access_token (40 мин, авто-обновление)
- **Endpoints:** `/api/1.0/accounts`, `/api/1.0/transactions`
- **Cron:** `*/15 * * * *` — автосинхронизация через `sync_bank_accounts`
- **Сертификаты на VPS:** `/var/www/www-root/data/www/logist2/certs/`
...
### Автовозы (AutoTransport) — расширенный функционал

**Автосмена статуса авто:**
- При смене статуса автовоза на LOADED/IN_TRANSIT/DELIVERED → все авто → TRANSFERRED + дата передачи
- Сигнал `autotransport_post_save` в `core/signals.py`, функция `_mark_cars_as_transferred()`
- Поддержка кастомной даты через `_transfer_date_override`

**Кнопка "Загружен" + поле даты:**
- AJAX-кнопка на странице списка автовозов для статусов FORMED/DRAFT
- Endpoint `<pk>/mark-loaded/` в `AutoTransportAdmin.get_urls()`
- Шаблон `templates/admin/core/autotransport/change_list.html`

**Цветовая индикация инвойсов:**
- Серая (нет инвойсов), красная (не оплачены), жёлтая (частично), зелёная (оплачены)
- Градиент при разных статусах нескольких инвойсов

**Скрытие доставленных:** `get_changelist_instance` + `get_queryset` (только для changelist, не для change_form)

**Mass action:** "🚛 Присвоить статус Доставлен"

**Кликабельные авто:** Select2 теги — ссылки на карточки авто (target="_blank"); VIN в позициях инвойса тоже кликабельный
...
- Логистика (Car, Container, AutoTransport — с автопередачей авто, цветными инвойсами)
...
Настроена в `logist2/settings_security.py`. При добавлении новых CDN — добавить домен в соответствующую директиву (style-src, font-src, script-src).

Текущие разрешённые домены:
- `fonts.googleapis.com`, `fonts.gstatic.com` — Google Fonts
- `cdn.jsdelivr.net` — Chart.js
- `cdnjs.cloudflare.com` — библиотеки
- `unpkg.com` — Boxicons
