...
**21.10.2025 - Исправлено наследование даты разгрузки контейнера + оптимизация производительности:**

✅ **ПРОБЛЕМА РЕШЕНА:** Дата разгрузки контейнера теперь ВСЕГДА наследуется всеми автомобилями
   - Раскомментирован и улучшен код в `ContainerAdmin.save_model()` для обновления даты разгрузки
   - Убрано условие `if not car.unload_date:` - теперь обновляются ВСЕ автомобили
   - Улучшен метод `Container.sync_cars_after_warehouse_change()` - принудительное наследование даты
   - Улучшен метод `Container.sync_cars_after_edit()` - принудительное наследование даты
...
⚡ **ОПТИМИЗАЦИЯ ПРОИЗВОДИТЕЛЬНОСТИ:**
   - Использован `bulk_update()` вместо сохранения автомобилей в цикле
   - Временное отключение сигналов при массовом обновлении для ускорения операции
   - Массовое обновление инвойсов после обновления всех автомобилей (вместо обновления после каждого авто)
