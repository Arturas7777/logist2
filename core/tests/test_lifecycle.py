...
class CheckContainerStatusTest(TestCase):
    ...
    def test_not_all_transferred_keeps_status(self):
        # ВАЖНО: сначала создаём UNLOADED-авто. Если первым сохранить
        # TRANSFERRED-авто, сигнал увидит "1 из 1 transferred" и переключит
        # контейнер на TRANSFERRED до того, как мы успеем создать второй
        # (UNLOADED) авто. Порядок создания критичен для воспроизводимости.
        Car.objects.create(
            year=2023,
            brand="Audi",
            vin="CONTSTAT000000004",
            status="UNLOADED",
            container=self.container,
        )
        car_transferred = Car.objects.create(
            year=2023,
            brand="BMW",
            vin="CONTSTAT000000003",
            status="TRANSFERRED",
            container=self.container,
            transfer_date=timezone.now().date(),
        )
        check_container_status(car_transferred)
        self.container.refresh_from_db()
