...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--target-px", type=int, default=2560, help="Максимальная длинная сторона (default: 2560)")
        parser.add_argument("--quality", type=int, default=85, help="JPEG quality (default: 85)")
        parser.add_argument("--limit", type=int, default=0, help="Обработать не более N фото (0 = все)")
        parser.add_argument("--batch-log", type=int, default=50, help="Логировать прогресс каждые N фото")
        parser.add_argument("--dry-run", action="store_true", help="Только считает, ничего не пишет")
        parser.add_argument("--force", action="store_true", help="Пережимать даже если long-side уже <= target")
    ...
    def _process_one(
        self, obj, field_name: str, media_root: Path, *, target_px: int, quality: int, dry_run: bool, force: bool
    ) -> dict:
        out = {}
        ...
        try:
            ...
            with Image.open(src_path) as im:
                ...
                if dry_run:
                    ...
                    os.close(tmp_fd)
                    try:
                        im.save(tmp_path, "JPEG", quality=quality, optimize=True, progressive=True)
            ...
            out["errors"] = 1
            logger.warning("resize_photos: %s (id=%s) — ошибка: %s", src_path, getattr(obj, "id", "?"), e)
            return out
