...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Миграция услуг завершена успешно!"))
    ...
    def create_service_types(self):
        ...
        for name, description in service_types:
            ServiceType.objects.get_or_create(name=name, defaults={"description": description})
    ...
    def migrate_warehouse_services(self):
        ...
        ServiceType.objects.get(name="Хранение")
        handling_type = ServiceType.objects.get(name="Погрузка/Разгрузка")
        documents_type = ServiceType.objects.get(name="Документооборот")
    ...
    def migrate_car_services(self):
        ...
        ServiceType.objects.get(name="Перевозка")
        ServiceType.objects.get(name="Документооборот")
        ServiceType.objects.get(name="Таможенное оформление")
        ServiceType.objects.get(name="Дополнительные услуги")

        for car in Car.objects.all():
            # Океанский фрахт
            if car.ocean_freight and car.ocean_freight > 0 and car.line:
                line_service = LineService.objects.filter(line=car.line, name="Океанский фрахт").first()
                if line_service:
            ...
            if car.transport_kz and car.transport_kz > 0 and car.carrier:
                carrier_service = CarrierService.objects.filter(carrier=car.carrier, name="Перевозка").first()
                if carrier_service:
                    CarService.objects.get_or_create(
                        car=car,
            ...
            if car.unload_fee and car.unload_fee > 0 and car.warehouse:
                warehouse_service = WarehouseService.objects.filter(warehouse=car.warehouse, name="Разгрузка").first()
            ...
            if car.docs_fee and car.docs_fee > 0 and car.warehouse:
                warehouse_service = WarehouseService.objects.filter(
                    warehouse=car.warehouse, name="Оформление документов"
                ).first()
                if warehouse_service:
                    CarService.objects.get_or_create(
                        car=car,
            ...
            if car.transit_declaration and car.transit_declaration > 0 and car.warehouse:
                warehouse_service = WarehouseService.objects.filter(
                    warehouse=car.warehouse, name="Транзитная декларация"
                ).first()
                if warehouse_service:
                    CarService.objects.get_or_create(
                        car=car,
            ...
            if car.export_declaration and car.export_declaration > 0 and car.warehouse:
                warehouse_service = WarehouseService.objects.filter(
                    warehouse=car.warehouse, name="Экспортная декларация"
                ).first()
                if warehouse_service:
                    CarService.objects.get_or_create(
                        car=car,
            ...
            if car.extra_costs and car.extra_costs > 0 and car.warehouse:
                warehouse_service = WarehouseService.objects.filter(
                    warehouse=car.warehouse, name="Дополнительные расходы"
                ).first()
                if warehouse_service:
                    CarService.objects.get_or_create(
                        car=car,
            ...
            if car.complex_fee and car.complex_fee > 0 and car.warehouse:
                warehouse_service = WarehouseService.objects.filter(warehouse=car.warehouse, name="Комплекс").first()
                if warehouse_service:
                    CarService.objects.get_or_create(
                        car=car,
                        service_type="WAREHOUSE",
