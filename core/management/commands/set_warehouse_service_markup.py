...
Сохранение карточки склада в админке больше не затирает наценки существующих
машин (сигнал трогает только `custom_price` при реальном изменении цены).
Команда оставлена для скриптов: меняет ТОЛЬКО значение в каталоге через
`.update()` (QuerySet.update сигнал post_save НЕ вызывает). Существующие авто
не затрагиваются — изменение влияет только на вновь создаваемые услуги авто.
