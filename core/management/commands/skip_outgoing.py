...
SKIP_RULES = [
    ...
    (
        "Комиссия Revolut",
        lambda bt: bool(
            re.search(r"Company (Pro|Free) plan fee|Expenses app charges", bt.description or "", re.IGNORECASE)
        ),
    ),
    (
        "Внутренний перевод",
        lambda bt: bool(re.search(r"TARP SASKAITU|tarp saskaitu", bt.description or "", re.IGNORECASE)),
    ),
    (
        "Соц. налоги / госплатежи",
        lambda bt: bool(
            re.search(
    ...
    ("Терминал Клайпеда (Neto)", lambda bt: bool(re.search(r"LTKLJP[X]?\d", bt.description or ""))),
    ("FAAS (контейнерный терминал)", lambda bt: bool(re.search(r"FAAS\s+Nr\.", bt.description or "", re.IGNORECASE))),
    ("TTG (логистика)", lambda bt: bool(re.search(r"TTG\s*-\s*L\d", bt.description or ""))),
    ("OTR (логистика)", lambda bt: bool(re.search(r"\d{2}OTR-\d", bt.description or ""))),
    ("Терминал TERM", lambda bt: bool(re.search(r"^TERM\d{4}$", (bt.description or "").strip()))),
    ("Автовозы ATC", lambda bt: bool(re.search(r"^ATC\d{5,6}$", (bt.description or "").strip()))),
    ("Автовозы AVL", lambda bt: bool(re.search(r"^AVL-\d{7}$", (bt.description or "").strip()))),
    (
        "Топливо",
        lambda bt: bool(
            re.search(
                r"Alau[sš]a|Orlen|Circle K|VIADA|Baltic Petroleum|Degalin[eė]",
                (bt.counterparty_name or "") + " " + (bt.description or ""),
                re.IGNORECASE,
            )
        ),
    ),
    ...
    ("Подписка site.pro", lambda bt: bool(re.search(r"^B1S\d{7}$", (bt.description or "").strip()))),
    (
        "Бухгалтерия / поставщики (номерные инвойсы)",
        lambda bt: bool(re.search(r"^\d{2}/\d{4,5}$", (bt.description or "").strip())),
    ),
    ("IMP INV (Maersk / импорт)", lambda bt: bool(re.search(r"IMP INV \d", bt.description or ""))),
    ("MAI (поставщик)", lambda bt: bool(re.search(r"^MAI M\d{6}$", (bt.description or "").strip()))),
    ("Комиссия Paysera", lambda bt: ("paysera" in (bt.counterparty_name or "").lower())),
    ("INVOICE поставщику", lambda bt: bool(re.search(r"^INVOICE\s+\d+/\d+TR$", (bt.description or "").strip()))),
    ("INVOICE - L (поставщик)", lambda bt: bool(re.search(r"^INVOICE\s*-\s*L\d", (bt.description or "").strip()))),
    ("WE (складские)", lambda bt: bool(re.search(r"^WE\s+\d{2}-\d{4}$", (bt.description or "").strip()))),
]
    ...
    def handle(self, *args, **options):
        ...
        if unmatched:
            ...
            for bt in unmatched:
                ...
                self.stdout.write(f"    {bt.created_at.strftime('%Y-%m-%d')} {bt.amount:>10} EUR  {cp:30} | {desc}")
