    ...
    def handle(self, *args, **options):
        ...
        if amount_mismatches:
            ...
            for m in amount_mismatches:
                self.stdout.write(
                    f"    {m['number']}: Django={m['django_total']}, site.pro={m['sitepro_total']}, разница={m['diff']}"
                )
        else:
        ...
        unpaid = NewInvoice.objects.filter(status__in=OPEN_INVOICE_STATUSES).order_by("date")
