...
class Command(BaseCommand):
    ...
    def check_indexes(self):
        ...
        with connection.cursor() as cursor:
            ...
            if index_count < 30:
                self.stdout.write(self.style.WARNING("   ⚠️  Мало индексов! Возможно, миграции не применены."))
                self.stdout.write(self.style.WARNING("   Выполните: python manage.py migrate"))
            else:
    ...
    def check_connection_pooling(self):
        ...
        if conn_max_age > 0:
            self.stdout.write(self.style.SUCCESS(f"   ✅ Connection pooling включен (CONN_MAX_AGE={conn_max_age}s)"))
        else:
            self.stdout.write(self.style.WARNING("   ⚠️  Connection pooling отключен"))
            self.stdout.write(self.style.WARNING("   Добавьте CONN_MAX_AGE в settings.py"))
    ...
    def validate_balances(self):
        ...
        for model in [Client, Warehouse, Line, Company, Carrier]:
            ...
            for entity in model.objects.all():
                ...
                if not validation["is_valid"]:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"   ❌ {model_name} #{entity.id} ({entity}): {', '.join(validation['issues'])}"
                        )
                    )
        ...
        if issues_found > 0:
            self.stdout.write(self.style.ERROR(f"   ❌ Найдено проблем: {issues_found}"))
            self.stdout.write(self.style.WARNING("   Запустите команду без --validate-only для исправления"))
        else:
            self.stdout.write(self.style.SUCCESS("   ✅ Все балансы консистентны"))
    ...
    def recalculate_balances(self):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"   ✅ Пересчитано сущностей: {result['entities_updated']}"))
            self.stdout.write(f"   ⏱️  Время выполнения: {elapsed:.2f}s")

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"   ❌ Ошибка пересчета: {e}"))
