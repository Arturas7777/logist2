...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cars = (
            Car.objects.select_related("container", "container__line", "warehouse", "line")
    ...
    def process_car(self, car, dry_run):
        ...
        if car.container and car.container.line:
            ...
            if new_line_service:
                ...
                if old_services != [new_line_service.id]:
                    ...
                    self.stdout.write(
                        f"  [LINE] {car.vin}: услуга линии -> {new_line_service.name} ({new_line_service.default_price})"
                    )
        ...
        if car.warehouse:
            ...
            if warehouse_services:
                old_services = set(
                    car.car_services.filter(service_type="WAREHOUSE").values_list("service_id", flat=True)
                )
