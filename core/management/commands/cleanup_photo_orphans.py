...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--only",
            type=str,
            choices=list(PHOTO_MODELS.keys()),
            default=None,
    ...
    def handle(self, *args, **opts):
        ...
        if opts["dry_run"] and opts["no_dry_run"]:
            self.stdout.write(self.style.WARNING("Указаны и --dry-run, и --no-dry-run; работаю в dry-run."))
            dry_run = True
        ...
        models_to_process = {opts["only"]: PHOTO_MODELS[opts["only"]]} if opts["only"] else PHOTO_MODELS

        totals = {"scanned": 0, "candidates": 0, "deleted": 0, "bytes_freed": 0, "errors": 0}
        ...
        for label, model in models_to_process.items():
            ...
            all_db_paths = set(model.objects.exclude(photo="").values_list("photo", flat=True))
            ...
            for cp in qs.iterator():
                ...
                orig_rel_path = os.path.join(db_dir, orig_name) if db_dir else orig_name
                ...
                if dry_run:
                    if candidates <= 5:
                        self.stdout.write(f"  ORPHAN: {orig_rel_path} ({size // 1024} KB) [{label}#{cp.id} → {fname}]")
                    bytes_freed += size
                    continue
                ...
                try:
                    ...
                    errors += 1
                    logger.warning("Не удалось удалить %s: %s", orig_abs_path, e)
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"DRY-RUN: ничего не удалено. Можно освободить: {mb:.1f} MB ({gb:.2f} GB)")
            )
            self.stdout.write(
                self.style.NOTICE("Запустить реально:  python manage.py cleanup_photo_orphans --no-dry-run")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"Удалено: {totals['deleted']}, освобождено: {mb:.1f} MB ({gb:.2f} GB)")
            )
            if totals["errors"]:
                self.stdout.write(self.style.ERROR(f"Ошибок при удалении: {totals['errors']}"))
