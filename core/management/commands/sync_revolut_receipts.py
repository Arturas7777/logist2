...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--days", type=int, default=30, help="За сколько дней тянуть expenses (по умолчанию 30)")
        parser.add_argument(
            "--missing-only",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for conn in qs:
            ...
            try:
                if missing_only:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"  Догружено чеков: {downloaded}"))
                else:
                    updated = service.fetch_expenses(days=days)
                    total_updated += updated
                    self.stdout.write(self.style.SUCCESS(f"  Обновлено expenses: {updated}"))
        ...
        if missing_only:
            self.stdout.write(
                self.style.SUCCESS(f"Итого догружено чеков: {total_downloaded}{f', {errors} ошибок' if errors else ''}")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"Итого обновлено: {total_updated}{f', {errors} ошибок' if errors else ''}")
            )
