...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--jobs", action="store_true", help="Update vin_validations on ScanProcessingJob")
        parser.add_argument("--cars", action="store_true", help="Print suspicious Car VINs (БД не меняется)")
        parser.add_argument("--limit", type=int, default=0, help="Limit number of items processed")
        parser.add_argument("--no-nhtsa", action="store_true", help="Use only check digit (faster, offline)")

    def handle(self, *args, **opts):
        if not opts["jobs"] and not opts["cars"]:
            self.stdout.write(self.style.WARNING("Specify --jobs and/or --cars"))
            return
    ...
    def _validate_jobs(self, *, limit, use_nhtsa):
            ...
            if cands and any(not c.get("validation") for c in cands):
                ...
                for c in cands:
                    ...
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f"  candidate {c.get('vin')} validation failed: {e}"))
            if changed:
        ...
        self.stdout.write(
            self.style.SUCCESS(f"\nJobs: total={n_total}, updated={n_updated}, with_warnings={n_warnings}")
        )
    ...
    def _report_cars(self, *, limit, use_nhtsa):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Cars: total={n_total}, bad_checksum_NA={n_bad_checksum}, nhtsa_warnings={n_nhtsa_bad}")
        )
        if suspicious:
            self.stdout.write("\nSuspicious VINs:")
