...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not company:
            self.stdout.write(self.style.ERROR("Компания Caromoto не найдена! Создайте компанию сначала."))
            return
        ...
        if existing:
            self.stdout.write(
                self.style.WARNING(f"  Найдено существующее подключение: {existing.name}\n  Оно будет обновлено.\n")
            )

        # Создаём/обновляем подключение
        conn_name = input('  Название подключения (по умолчанию "Site.pro Caromoto"): ').strip()
        if not conn_name:
        ...
        auto_push = input("  Автоматически отправлять инвойсы при выставлении? (y/n, по умолчанию n): ").strip().lower()
