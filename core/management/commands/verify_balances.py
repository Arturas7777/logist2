...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--entity', choices=ENTITY_CHOICES, default=None,
            help='Проверить только одну группу сущностей',
        )
        parser.add_argument(
            '--no-invoices', action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if check_invoices:
            ...
            if not invoice_mismatches:
                ...
                for m in invoice_mismatches:
                    self.stdout.write(self.style.WARNING(
                        f"  Инвойс {m['number']} (id={m['pk']}): "
                        f"stored={m['stored']}, expected={m['expected']}"
                    ))
                if fix:
        ...
        if total == 0:
            ...
            self.stdout.write(self.style.WARNING(
                f'Найдено расхождений: {total}. Запустите с --fix для исправления.'
            ))
