...
class Command(BaseCommand):
    help = (
        'Перегенерирует JWT-assertion для Revolut-подключения '
        'из существующего приватного ключа (без OAuth-flow).'
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=None,
            help='ID BankConnection (если несколько Revolut-подключений). '
                 'По умолчанию — единственное активное.',
        )
        parser.add_argument(
            '--redirect-uri',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if not client_id:
            raise CommandError(
                f'У {conn} пустой client_id. Сделайте полный setup_revolut.'
            )
        ...
        conn.save(update_fields=[
            '_jwt_assertion', 'access_token_expires_at',
            'last_error', 'updated_at',
        ])

        new_exp = conn.jwt_expires_at
        self.stdout.write(self.style.SUCCESS(
            f'  JWT обновлён, новый срок: {new_exp.isoformat() if new_exp else "?"}'
        ))
    ...
    def _generate_jwt(self, private_key_path: str, client_id: str,
                      redirect_uri: str, days: int) -> str:
