...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        redirect_uri = input(
            "  Введите OAuth Redirect URI (по умолчанию https://caromoto-lt.com/api/revolut/callback/): "
        ).strip()
        if not redirect_uri:
        ...
        consent_url = (
            f"{app_url}/app-confirm?client_id={client_id}&redirect_uri={redirect_uri}&response_type=code&scope=READ"
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  Access token получен (истекает через {expires_in}с)\n  Refresh token получен\n")
        )
