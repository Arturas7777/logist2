...
class Command(BaseCommand):
        ...
        for stype, model_cls in model_map.items():
            ...
            for cs in car_services.iterator():
                if cs.service_id not in valid_ids:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"  Orphan: CarService id={cs.id} car={cs.car_id} type={stype} service_id={cs.service_id}"
                        )
                    )
                    if fix:
                        cs.delete()
    ...
    def _check_invoice_paid_amounts(self, fix):
        ...
        for inv in invoices.iterator():
            payments = inv.transactions.filter(type="PAYMENT", status="COMPLETED").aggregate(total=Sum("amount"))[
                "total"
            ] or Decimal("0.00")
            refunds = inv.transactions.filter(type="REFUND", status="COMPLETED").aggregate(total=Sum("amount"))[
                "total"
            ] or Decimal("0.00")
    ...
    def _check_invoice_parties(self):
        ...
        for inv in NewInvoice.objects.exclude(status="CANCELLED").iterator():
            issuers = sum(
                1
                for f in [
                    inv.issuer_company_id,
                    inv.issuer_warehouse_id,
                    inv.issuer_line_id,
                    inv.issuer_carrier_id,
                ]
                if f
            )
            recipients = sum(
                1
                for f in [
                    inv.recipient_client_id,
                    inv.recipient_warehouse_id,
                    inv.recipient_line_id,
                    inv.recipient_carrier_id,
            ...
            if issuers != 1:
                issues += 1
                self.stdout.write(self.style.WARNING(f"  Invoice {inv.number}: has {issuers} issuers (expected 1)"))
            if recipients != 1:
                issues += 1
                self.stdout.write(
                    self.style.WARNING(f"  Invoice {inv.number}: has {recipients} recipients (expected 1)")
                )
