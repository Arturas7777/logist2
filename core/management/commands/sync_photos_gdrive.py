...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--delay-hours", type=int, default=12, help="Задержка в часах после статуса UNLOADED (по умолчанию 12)"
        )
        parser.add_argument(
            "--no-photos", action="store_true", help="[BEST] Проверить только контейнеры БЕЗ фотографий (быстрый режим)"
        )
        parser.add_argument(
            "--all", action="store_true", help="Синхронизировать все контейнеры (сканирует всю структуру Google Drive)"
        )
        parser.add_argument(
            "--recent", action="store_true", help="Синхронизировать недавние контейнеры (за последние N дней)"
        )
        parser.add_argument(
            "--days", type=int, default=14, help="Количество дней для --recent и --no-photos (по умолчанию 14)"
        )
        parser.add_argument("--container", type=str, help="Синхронизировать конкретный контейнер по номеру")
        parser.add_argument("--limit", type=int, help="Максимальное количество контейнеров для обработки (для тестов)")
