    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            if options["force"]:
                ...
                company.save()
                self.stdout.write(self.style.WARNING(f"Обновлена компания: {company.name}"))
            else:
                self.stdout.write(self.style.WARNING(f"Компания уже существует: {company.name}"))
        ...
        for balance_type in ["INVOICE", "CASH", "CARD"]:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"Создан {balance_type} баланс для компании"))
        ...
        for client in clients:
            for balance_type in ["INVOICE", "CASH", "CARD"]:
                ...
                if created:
                    self.stdout.write(f"  Создан {balance_type} баланс для клиента {client.name}")
        ...
        for line in lines:
            for balance_type in ["INVOICE", "CASH", "CARD"]:
                ...
                if created:
                    self.stdout.write(f"  Создан {balance_type} баланс для линии {line.name}")
        ...
        for warehouse in warehouses:
            for balance_type in ["INVOICE", "CASH", "CARD"]:
                ...
                if created:
                    self.stdout.write(f"  Создан {balance_type} баланс для склада {warehouse.name}")

        self.stdout.write(self.style.SUCCESS("Инициализация новой системы балансов завершена успешно!"))
        ...
        company_balances = Balance.objects.filter(content_type=company_content_type, object_id=company.id)
