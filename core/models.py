...
class ClientTariff(models.Model):
    """
    Тариф клиента — согласованная цена за авто (без хранения).
    Хранение всегда считается отдельно (дни × ставка).
    """
    TARIFF_TYPES = [
        ('FIXED', 'Фиксированная цена за авто'),
        ('VOLUME', 'Зависит от кол-ва ТС в контейнере'),
    ]
    
    client = models.OneToOneField(Client, on_delete=models.CASCADE,
                                  related_name='tariff', verbose_name="Клиент")
    tariff_type = models.CharField(max_length=10, choices=TARIFF_TYPES,
                                   verbose_name="Тип тарифа")
    fixed_price = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True,
        verbose_name="Фикс. цена за авто (без хранения)",
        help_text="Используется при типе 'Фиксированная цена'"
    )
    notes = models.TextField(blank=True, verbose_name="Примечания")
    is_active = models.BooleanField(default=True, verbose_name="Активен")
    ...
    def __str__(self):
        if self.tariff_type == 'FIXED':
            return f"{self.client.name} — фикс. {self.fixed_price}€/авто"
        return f"{self.client.name} — объёмный тариф"
    
    def get_price_per_car(self, total_cars_in_container):
        """
        Возвращает согласованную цену за авто (без хранения) или None.
        
        Args:
            total_cars_in_container: ОБЩЕЕ количество ТС в контейнере (от всех клиентов)
        """
        ...
        # VOLUME — ищем подходящий тир
        tier = self.tiers.filter(
            min_cars__lte=total_cars_in_container
        ).filter(
            models.Q(max_cars__gte=total_cars_in_container) | models.Q(max_cars__isnull=True)
        ).order_by('-min_cars').first()
        
        return tier.price_per_car if tier else None


class ClientTariffTier(models.Model):
    """
    Ступень объёмного тарифа.
    Пример: 1-2 ТС в контейнере → 350€/авто, 3-4 → 300€, 5+ → 280€
    """
    tariff = models.ForeignKey(ClientTariff, on_delete=models.CASCADE,
                               related_name='tiers', verbose_name="Тариф")
    min_cars = models.PositiveIntegerField(verbose_name="От (всего ТС в контейнере)")
    max_cars = models.PositiveIntegerField(
        null=True, blank=True,
        verbose_name="До (всего ТС, пусто = и более)"
    )
    price_per_car = models.DecimalField(
        max_digits=10, decimal_places=2,
        verbose_name="Цена за авто (без хранения)"
    )
    ...
    def __str__(self):
        if self.max_cars:
            return f"{self.min_cars}-{self.max_cars} ТС → {self.price_per_car}€"
        return f"{self.min_cars}+ ТС → {self.price_per_car}€"
