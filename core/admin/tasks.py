...
class TaskAdmin(admin.ModelAdmin):
    ...
    def state_indicator(self, obj):
        if obj.is_completed:
            return format_html(
                '<span style="color:#10b981;font-size:18px;line-height:1;" title="Выполнено">✓</span>'
            )
        if obj.is_overdue:
            return format_html(
                '<span style="color:#dc2626;font-size:18px;line-height:1;" title="Просрочено">!</span>'
            )
        return format_html(
            '<span style="color:#6b7280;font-size:18px;line-height:1;" title="Открыто">○</span>'
        )
    ...
    def deadline_display(self, obj):
        ...
        if obj.is_overdue:
            return format_html(
                '<span style="color:#dc2626;font-weight:700;" title="Просрочено">⏰ {}</span>',
                formatted
            )
    ...
    def related_object(self, obj):
        ...
        if obj.car_id:
            ...
            parts.append(format_html(
                ...
                '🚗 {}</a>', url, obj.car.vin if obj.car else f'#{obj.car_id}'
            ))
        if obj.container_id:
            ...
            parts.append(format_html(
                ...
                '📦 {}</a>', url, obj.container.number if obj.container else f'#{obj.container_id}'
            ))
        if not parts:
            return format_html('<span style="color:#9ca3af;">—</span>')
    ...
    def completed_display(self, obj):
        ...
        return format_html(
            '<span style="color:#10b981;font-size:12px;" title="Выполнил: {}">{}</span>',
            by, date_str
        )
