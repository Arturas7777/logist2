...
class ContainerEmailAdmin(admin.ModelAdmin):
    ...
    def direction_badge(self, obj: ContainerEmail) -> str:
        if obj.direction == ContainerEmail.DIRECTION_OUTGOING:
            return format_html('<span style="color:#2563eb;font-weight:600;" title="Исходящее">→</span>')
        return format_html('<span style="color:#16a34a;font-weight:600;" title="Входящее">←</span>')
    ...
    def action_attach_to_container(self, request, queryset):
        if "apply" in request.POST:
            ...
            if form.is_valid():
                ...
                self.message_user(
                    request,
                    f"Привязано {len(email_ids)} писем к контейнеру {container}",
                    level=messages.SUCCESS,
                )
                return redirect(request.get_full_path())
        else:
            form = AttachToContainerForm(
                initial={
        ...
        return render(
            request,
            "admin/core/containeremail/attach_to_container.html",
            {
                "form": form,
                "emails": queryset,
