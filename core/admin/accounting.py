...
from core.models.accounting import SiteProConnection, SiteProInvoiceSync
...
class SiteProConnectionAdmin(admin.ModelAdmin):
    ...
    def last_error_display(self, obj):
        if obj.last_error:
            ...
            return format_html('<span style="color: #dc3545;" title="{}">{}</span>', obj.last_error, error_text)
    ...
    def test_connection(self, request, queryset):
        ...
        for conn in queryset:
            ...
            if result["success"]:
                ...
                messages.success(
                    request, f"{conn.name}: подключение успешно! Метод: {result['auth_method']}. Деталі: {details}"
                )
            else:
    ...
    def load_reference_books(self, request, queryset):
        ...
        for conn in queryset:
            ...
            try:
                ...
                if wh:
                    wh_rows = [
                        f"<code>id={w['id']} name={w.get('name')} isPrimary={w.get('isPrimary')}</code>" for w in wh
                    ]
                ...
                if sale_ops:
                    op_rows = [f"<code>id={o['id']} code={o.get('code')} name={o.get('name')}</code>" for o in sale_ops]
                ...
                active_items = [
                    it
                    for it in (items_ref.get("data", []) if isinstance(items_ref, dict) else [])
                    if it.get("isActive")
                ]
                if active_items:
                    it_rows = [
...
class SiteProInvoiceSyncAdmin(admin.ModelAdmin):
    ...
    def error_display(self, obj):
        if obj.error_message:
            ...
            return format_html('<span style="color: #dc3545;" title="{}">{}</span>', obj.error_message, error_text)
        return "-"
