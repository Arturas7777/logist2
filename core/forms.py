...
class WarehouseForm(ModelForm):
    ...
    def save(self, commit=True):
        ...
        if commit and instance.pk:
                ...
                if field_name.startswith("new_service_name_"):
                    ...
                    if name:
                        try:
                            ...
                            WarehouseService.objects.create(warehouse=instance, name=name, default_price=decimal_price)
            ...
            for field_name, value in self.cleaned_data.items():
                if (
                    field_name.startswith("service_")
                    and not field_name.startswith("delete_service_")
                    and not field_name.startswith("new_service_")
                ):
                    service_id = field_name.replace("service_", "")
                    if service_id.isdigit():
                        try:
