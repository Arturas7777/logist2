...
def get_car_profitability(car_ids=None, container_ids=None, audit_ids=None):
    ...
    if car_ids or container_ids:
        ...
        if container_ids:
            container_car_set = set(Car.objects.filter(container_id__in=container_ids).values_list("id", flat=True))
    ...
    confirmed_svc_by_car = dict(
        SupplierCost.objects.filter(car_id__in=target_car_ids, car_service__isnull=False)
        .values("car_id")
        .annotate(cnt=Count("car_service_id", distinct=True))
        .values_list("car_id", "cnt")
    )
    # Unlinked SupplierCost per car (car_service=None, needs review)
    unlinked_by_car = dict(
        SupplierCost.objects.filter(car_id__in=target_car_ids, car_service__isnull=True)
        .values("car_id")
        .annotate(cnt=Count("id"))
        .values_list("car_id", "cnt")
    )
        ...
        result.append(
            {
                ...
                if hasattr(car, "get_vehicle_type_display")
...
def generate_hints(audit_ids=None):
    ...
    cost_qs.filter(car__isnull=False, car_service__isnull=True).values_list("counterparty", flat=True).distinct()
...
def get_unlinked_costs(audit_ids=None):
    ...
    qs = SupplierCost.objects.filter(car__isnull=False, car_service__isnull=True).select_related("car", "audit")
    if audit_ids:
