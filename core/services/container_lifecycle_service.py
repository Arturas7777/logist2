...
def bulk_update_car_statuses(container) -> None:
    ...
    try:
        ...
        logger.info("Updated status to '%s' for %s cars in container %s", container.status, updated_count, container.number)
    except Exception as e:
...
def apply_unload_date_change(container) -> None:
    ...
    try:
        logger.info("Unload date changed for container %s to %s, bulk updating all cars...", container.id, container.unload_date)
...
def apply_ths_change(container, changed_data) -> None:
    ...
    try:
        ...
        logger.info(
            ...
            container.id, container.line, container.ths, container.ths_payer,
        )
        ...
        with signals_disabled(*(CAR_SIGNALS + INVOICE_SIGNALS)):
            ...
            if container.line and container.ths:
                ...
                deleted_line = CarService.objects.filter(
                    car_id__in=car_ids, service_type='LINE'
                ).filter(
                ...
                deleted_wh = CarService.objects.filter(
                    car_id__in=car_ids, service_type='WAREHOUSE'
                ).filter(
                    service_id__in=WarehouseService.objects.filter(name__icontains='THS').values_list('id', flat=True)
                ).delete()
                logger.info("[TIMING] Deleted %s line THS and %s warehouse THS services", deleted_line[0], deleted_wh[0])
