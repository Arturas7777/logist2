...
class RevolutService:
        ...
        while True:
            try:
                ...
                if resp.status_code == 429 and attempt < max_retries:
                    ...
                    logger.info(f"[Revolut] 429 Rate limit на {endpoint}, пауза {retry_after}с")
        ...
        for item in data[:limit]:
            ...
            if legs:
                ...
                if isinstance(cp, dict):
                    counterparty = cp.get("account_name", "") or cp.get("name", "") or cp.get("company_name", "")
            ...
            if not counterparty:
                ...
                if isinstance(top_cp, dict):
                    counterparty = (
                        top_cp.get("name", "") or top_cp.get("account_name", "") or top_cp.get("company_name", "")
                    )
    ...
    def fetch_expenses(self, days: int = 30, limit: int = 500) -> int:
        ...
        try:
            ...
            if e.status_code == 403:
                logger.warning("[Revolut] Expenses API недоступен (403): возможно, план ниже Grow")
                return 0
            raise
    ...
    def _download_receipt(self, bt, expense_id: str, receipt_id: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"[Revolut] Не удалось скачать чек {receipt_id} для BT {bt.pk}: {e}")
            return False
        ...
        logger.info(f"[Revolut] Чек сохранён: BT {bt.pk} → {filename} ({len(body)} байт)")
        return True
    ...
    def fetch_receipts_for_existing(self, limit: int = 200) -> int:
        ...
        qs = (
            ...
            .filter(Q(receipt_file="") | Q(receipt_file__isnull=True))
            .exclude(expense_id="")
