...
def _collect_processes() -> dict[str, dict[str, Any]]:
    ...
    groups: dict[str, dict[str, Any]] = {name: {"rss_mb": 0, "count": 0} for name in _PROCESS_GROUPS}
    ...
    try:
        ...
        with connection.cursor() as cur:
            ...
            try:
                ...
                result["slow_queries"] = [dict(zip(cols, row, strict=False)) for row in cur.fetchall()]
            except Exception as exc:
                logger.debug("pg_stat_statements not available: %s", exc)
                result["slow_queries"] = []
                result["slow_queries_error"] = "pg_stat_statements не настроен. Включите расширение в postgresql.conf."
