...
def company_dashboard(request):
    ...
    context["cars_by_status_json"] = {k: v for k, v in context["cars_by_status"].items() if k != "total"}
...
def add_cash_expense(request):
    ...
    personal_categories = ExpenseCategory.objects.filter(category_type="PERSONAL", is_active=True).order_by("order")
...
def expense_analytics(request):
    ...
    trend = svc.get_monthly_trend(months={"1m": 1, "3m": 3, "6m": 6, "1y": 12, "all": 24}.get(period, 3))
...
def upload_expense_receipt(request, tx_id):
    ...
    if request.method == "POST":
        ...
        if ext not in ALLOWED_EXTENSIONS:
            messages.error(
                request, f"Недопустимый тип файла .{ext}. Разрешены: {', '.join(sorted(ALLOWED_EXTENSIONS))}"
            )
            return redirect("company_dashboard")
