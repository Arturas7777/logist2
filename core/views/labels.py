...
def _load_containers(ids: list[int]) -> list[Container]:
    ...
    if not ids:
        return []
    qs = (
        Container.objects.filter(id__in=ids)
        .select_related("line")
        .prefetch_related("container_cars__client", "container_cars__line")
    )
...
def _build_label_data(container: Container) -> dict[str, Any]:
    ...
    printed_str = (
        ...
        if container.labels_printed_at
        else ""
    )
...
def print_labels_sheet(request) -> HttpResponse:
    ...
    if containers:
        ...
        Container.objects.filter(id__in=printed_ids).update(labels_printed_at=timezone.now())
