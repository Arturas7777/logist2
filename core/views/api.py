...
def car_list_api(request):
    ...
    if client_id_int:
        ...
        all_cars = (
            Car.objects.by_client(client_id_int)
        ...
        if search_query:
            ...
            if search_query.isdigit():
                try:
                    ...
                    year_q = Q()
            all_cars = all_cars.filter(Q(vin__icontains=search_query) | Q(brand__icontains=search_query) | year_q)
...
def search_partners_api(request):
    ...
    try:
        ...
        objects_list = [{"id": obj.id, "name": getattr(obj, "name", str(obj)), "type": entity_type} for obj in objects]
...
def get_invoice_cars_api(request):
    ...
    try:
        ...
        if from_entity_type == "CLIENT":
            ...
            cars = Car.objects.filter(
                Q(status__in=["UNLOADED", "IN_PORT", "TRANSFERRED"]) & Q(unload_date__gte=six_months_ago)
            ).select_related("client", "warehouse", "container", "line", "carrier")
        ...
        if search_query:
            ...
            if search_query.isdigit():
                try:
                    ...
                    year_q = Q()
            cars = cars.filter(Q(vin__icontains=search_query) | Q(brand__icontains=search_query) | year_q)
        ...
        if car_ids:
            ...
            rows = (
                CarService.objects.filter(car_id__in=car_ids, service_type__in=("LINE", "CARRIER"))
...
def get_warehouse_cars_api(request):
    ...
    try:
        ...
        cars = Car.objects.filter(
            Q(warehouse_id=warehouse_id) & Q(status__in=["UNLOADED", "TRANSFERRED"]) & Q(unload_date__gte=month_ago)
        ).select_related("client", "warehouse", "container", "line", "carrier")
    ...
    try:
        ...
        if service_type == "warehouse":
            ...
            existing_service_ids = set(
                CarService.objects.filter(car=car, service_type="WAREHOUSE").values_list("service_id", flat=True)
            )
            services = [
            ...
            existing_service_ids = set(
                CarService.objects.filter(car=car, service_type="LINE").values_list("service_id", flat=True)
            )
            services = [
                {"id": s.id, "name": s.name, "price": float(s.default_price)}
            ...
            existing_service_ids = set(
                CarService.objects.filter(car=car, service_type="CARRIER").values_list("service_id", flat=True)
            )
            services = [
                {"id": s.id, "name": s.name, "price": float(s.default_price)}
            ...
            existing_service_ids = set(
                CarService.objects.filter(car=car, service_type="COMPANY").values_list("service_id", flat=True)
            )
            services = [
                {"id": s.id, "name": s.name, "price": float(s.default_price)}
                for s in CompanyService.objects.filter(company=company, is_active=True).exclude(
...
def add_services(request, car_id):
    ...
    try:
        ...
        existing_ids = set(
            CarService.objects.filter(car=car, service_type=service_type_upper, service_id__in=service_ids).values_list(
        ...
        if to_create:
            ...
            added_count = CarService.objects.filter(
                car=car, service_type=service_type_upper, service_id__in=[s.service_id for s in to_create]
