...
def comparison_dashboard(request):
    ...
    discrepancies = [
        ...
        for c in client_comparisons
        if c["status"] not in ("match", "no_data")
    ] + [
        {"type": "warehouse_comparison", "entity": w["warehouse_name"], "comparison": w}
        for w in warehouse_comparisons
        if w["status"] not in ("match", "no_data")
    ]
...
def compare_client_costs_api(request):
    ...
    try:
        ...
        result = ComparisonService().compare_client_costs_with_warehouse_invoices(client, start_date, end_date)
        return JsonResponse(result)
...
def compare_warehouse_costs_api(request):
    ...
    try:
        ...
        result = ComparisonService().compare_warehouse_costs_with_payments(warehouse, start_date, end_date)
        return JsonResponse(result)
