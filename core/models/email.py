...
class ContainerEmail(models.Model):
    ...
    sent_from_container = models.ForeignKey(
        ...
        help_text="Контейнер, из карточки которого было отправлено письмо (только для OUTGOING).",
    )
...
class EmailGroupMember(models.Model):
    ...
    display_name = models.CharField(
        ...
        help_text='Если заполнено, в поле «Кому» подставится как "Имя <email@...>". Иначе — только email.',
    )
