...
### Fixed — FIXED-тариф: складской пакет ровно равен тарифу (2026-06-17)

- Для клиентов с тарифом `FIXED` складской пакет (услуги склада без хранения)
  теперь приводится **ровно** к сумме тарифа. Раньше дефолтная скрытая наценка
  услуг склада (`WarehouseService.default_markup`) задирала пакет выше тарифа:
  логика `_distribute_markup_for_car` считала «уже ≥ тарифа» и наценку не
  трогала (пример: тариф 260 € + наценка 20 € → пакет 265 € вместо 260 €).
- Новая логика для `FIXED`: скрытая наценка всегда сбрасывается и добавляется
  ровно до тарифа; если базовые цены услуг уже ≥ тарифа (доп. услуги,
  повышенный THS) — наценка обнуляется, но реальные цены не дисконтируются в
  минус. Поведение `FLEXIBLE` (тариф = «пол») без изменений.
- Хранение по-прежнему добавляется сверх тарифа.
