...
- Новая логика `_distribute_markup_for_car` для `FIXED`: скрытая наценка всегда
  ...
  т.к. с клиента берётся только фиксированная сумма. `FLEXIBLE` (тариф = «пол»)
  без изменений. Хранение по-прежнему добавляется сверх тарифа.
...
- Команда `recalculate_fixed_tariffs` (`--dry-run`, `--all`) переприменяет
  тариф к существующим авто FIXED-клиентов; по умолчанию обрабатывает только
  активные (не `TRANSFERRED`), чтобы не менять историю.
