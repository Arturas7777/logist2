...
> Сделано: `0001_squashed_baseline` (replaces всех 178 миграций) + сохранены RunPython-данные (сид категорий расходов из 0112/0141, pg_trgm-индексы из 0186). Старые файлы миграций оставлены на переходный период — удалить отдельным коммитом после того, как прод пройдёт `migrate` (squashed пометится applied).
