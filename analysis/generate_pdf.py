...
H1 = ParagraphStyle("H1", fontName=FONT_B, fontSize=20, leading=24, spaceAfter=6, textColor=colors.HexColor("#0f172a"))
H2 = ParagraphStyle(
    "H2", fontName=FONT_B, fontSize=14, leading=18, spaceBefore=12, spaceAfter=6, textColor=colors.HexColor("#0f172a")
)
H3 = ParagraphStyle(
    "H3", fontName=FONT_B, fontSize=11, leading=14, spaceBefore=6, spaceAfter=4, textColor=colors.HexColor("#334155")
)
P = ParagraphStyle("P", fontName=FONT, fontSize=10, leading=14, spaceAfter=4, textColor=colors.HexColor("#1f2937"))
SMALL = ParagraphStyle("SMALL", fontName=FONT, fontSize=8.5, leading=11, textColor=colors.HexColor("#64748b"))
NOTE = ParagraphStyle(
    "NOTE", fontName=FONT_I, fontSize=9, leading=12, textColor=colors.HexColor("#475569"), leftIndent=8, spaceAfter=4
)
TOTAL = ParagraphStyle("TOTAL", fontName=FONT_B, fontSize=15, leading=20, textColor=colors.HexColor("#15803d"))
...
incomes_table_data = [
    ...
    ["28.01.2025", "Revolut USD", fmt("1166.63"), "DANIEL SOLTYS", "1 215,75 USD → курс LB 1,0421"],
...
excluded_table_data = [
    ...
    [
        "Возвраты покупок (refund/grąžinimas)",
        fmt("1892.87"),
        "12",
        "Возврат денег за товар (Auvika UAB 1 874,16 € + микрорефанды).",
    ],
    ["Пособие на ребёнка (Išmoka vaikui)", fmt("1443.75"), "12", "Социальная выплата — не относится к доходу IV."],
    ["Муниципальная компенсация", fmt("1000.00"), "1", "Дотация Ignalinos savivaldybė за септик — не доход IV."],
    ["Выигрыши Optibet", fmt("319.00"), "2", "Декларируются отдельно (код 42, GPM311)."],
    [
        "Парная FX-сторона Revolut",
        fmt("1163.67"),
        "1",
        "EUR-сторона FX-обмена 1 215,75 USD → EUR (доход уже учтён в USD).",
    ],
    ["Внутренние операции Revolut", fmt("1080.00"), "2", "Перемещение между Vault/Pockets/Savings."],
]
...
scenarios = [
    {
        ...
        "total": GPM + VSD + PSD,
        "note": (
            "Работает только если за 2025 год вы НЕ платили ежемесячные "
    ...
    },
    {
        ...
        "rows": [
            (f"Уже уплачено за год: VSD min ({fmt(VSD_MIN_MONTHLY)} × 12)", fmt(VSD_MIN_ANNUAL)),
            (f"Уже уплачено за год: PSD min ({fmt(PSD_MIN_MONTHLY)} × 12)", fmt(PSD_MIN_ANNUAL)),
        ...
        "note": (
            "В этом случае фактический налоговый «бюджет» за 2025 год — это сумма "
    ...
    },
    {
        ...
        "rows": [
            ...
            ("PSD по IV (доплата при декларации, если работодатель не покрывал IV)", fmt(PSD)),
        ],
        "total": GPM + VSD + PSD,
        "note": (
            "Если работодатель полностью покрывал PSD по основному месту — "
...
def make_table(
    ...
    style_cmds = [
        ...
        ("LINEBELOW", (0, "splitlast"), (-1, "splitlast"), 0.25, colors.HexColor("#e2e8f0")),
    ...
    if highlight_row_idx is not None:
        style_cmds.append(("BACKGROUND", (0, highlight_row_idx), (-1, highlight_row_idx), colors.HexColor("#fef3c7")))
...
def header_footer(canvas, doc):
    ...
    canvas.drawString(20 * mm, 10 * mm, "Доход IV 2025 · подготовлено для GPM311 · автогенерация")
...
def build():
    ...
    doc = SimpleDocTemplate(
        ...
        subject="GPM311 / Individuali veikla 2025",
    )
    story: list = []
    ...
    story.append(
        Paragraph(
            ...
            "30 % нормативу расходов (без чеков), для IV pagal pažymą.",
            P,
        )
    )
    ...
    story.append(
        Paragraph(
            ...
            "Lietuvos banko на 28.01.2025 (1 EUR = 1,0421 USD).",
            SMALL,
        )
    )
    ...
    story.append(
        Paragraph(
            ...
            "(Arturas Haizhutsis / HAIZHUTSIS ARTURAS).",
            SMALL,
        )
    )
    ...
    story.append(
        Paragraph(
            "Применён нормативный вычет 30 % (без подтверждающих чеков). Apmokestinamosios pajamos = 70 % × gross.", P
        )
    )
    story.append(
        make_table(
    ...
    story.append(
        Paragraph(
            ...
            "(10 % от apm. pajamos), эффективная ставка GPM = 5 %.",
            SMALL,
        )
    )
    story.append(Spacer(1, 10))
    ...
    story.append(
        Paragraph(
            ...
            "Sodra и был ли у вас параллельный работодатель.",
            P,
        )
    )
    ...
    checklist = [
        ...
        "Проверить, нет ли других валютных «карманов» Revolut (PLN/BYN/GBP) с поступлениями.",
    ...
    story.append(
        Paragraph(
            ...
            "сверить в Mano VMI / Mano Sodra или с бухгалтером.",
            NOTE,
        )
    )
